...
def extra_node_config_file(algorithm_image):
    ...
    try:
        if original_content:
            ...
            print(f"Removed config file: {full_path}")
    except OSError as e:
...
def vantage6_network_session(docker_client, extra_node_config_file):
    ...
    try:
        ...
        if len(network_info["created_containers"]) < 4:
            pytest.skip(
                f"Network setup incomplete: only {len(network_info['created_containers'])} containers created (expected ≥4)")
...
def authentication(vantage6_network_session, docker_client) -> Client:
    ...
    # Create a client
    vantage6_client = Client(vantage6_config.get('server_url'), vantage6_config.get('server_port'),
    ...
    for attempt in range(max_retries):
        try:
            vantage6_client.authenticate(vantage6_config.get('username'), vantage6_config.get('password'))
            break
        except Exception as e:
            if attempt == max_retries - 1:
                pytest.skip(f"Failed to authenticate after {max_retries} attempts: {e}")
            print(f"Authentication attempt {attempt + 1} failed, retrying... Error: {e}")
            time.sleep(5)
