...
class TestProjectMiscellaneous:
    ...
    def test_algorithm_wiki_exists(self):
        ...
        if usage_rst.exists():
            ...
            function_indicators = ["central", "partial_general_statistics", "partial_aggregate_adjusted_deviation"]
            found_functions = [func for func in function_indicators if func in content]
            
            if found_functions:
                print(f"✓ Documentation found with function references: {found_functions}")
            else:
                print("⚠ Warning: usage.rst exists but may not contain function definitions")
        else:
            print("⚠ Note: usage.rst not found - consider adding algorithm usage documentation")
    ...
    def test_algorithm_store_json_exists(self):
        ...
        if not algorithm_store_file.exists():
            print("⚠ Warning: algorithm_store.json not found - this may be needed for algorithm store registration")
            return
        ...
        try:
            ...
            missing_fields = [field for field in required_fields if field not in store_config]
            
            if missing_fields:
                print(f"⚠ Warning: algorithm_store.json missing required fields: {missing_fields}")
            else:
